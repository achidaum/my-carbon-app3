...
# ส่วนเนื้อหารายงาน
st.info("""
**เป้าหมายของกิจกรรม:** รายงานฉบับนี้จัดทำขึ้นภายใต้โครงการการประเมินและส่งเสริมการลดก๊าซเรือนกระจกระดับบุคคล 
...
with col1:
    transport = st.selectbox("เลือกพาหนะ", ["รถยนต์ส่วนตัว (น้ำมัน)", "รถยนต์ไฟฟ้า (EV)", "รถจักรยานยนต์", "รถไฟฟ้า", "รถเมล์", "เดิน/ปั่นจักรยาน"])
    distance = st.number_input("ระยะทาง (กม.)", min_value=0.0)
with col2:
    food_input = st.text_input("เมนูอาหาร", placeholder="เช่น กะเพราไก่")
...
if st.button("ประเมินผลลัพธ์"):
    ...
    # แสดงผล Card (แก้จุดที่ Error เรียบร้อยแล้ว)
    st.markdown(f"""
        <div style="background: white; padding: 25px; border-radius: 20px; border-left: 10px solid #10b981; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
            ...
            <hr>
            <p style="color: #6b7280; font-size: 14px;">🍴 เมนู: {food_input if food_input else 'ไม่ได้ระบุ'}</p>
        </div>
    """, unsafe_allow_html=True)
